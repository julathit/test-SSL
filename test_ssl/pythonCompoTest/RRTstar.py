...
def rrt_star(start, goal, obstacle_list, bounds, num_iterations=1000, step_size=0.1, goal_tolerance=0.05, neighbor_radius=0.2):
...
if __name__ == "__main__":
    ...
    search_bounds = [-4500, 4500, -3000, 3000]  # [x_min, x_max, y_min, y_max]

    start_time = time.time()  
    # Run the RRT* algorithm
    rrt_tree, optimal_path = rrt_star(start_point, goal_point, obstacle_list, search_bounds, num_iterations=2000, step_size=50, neighbor_radius=0.1)
    end_time = time.time()
    print(end_time-start_time)

    # Plot the results
    plot_rrt_star(rrt_tree, optimal_path, start_point, goal_point, obstacle_list, search_bounds)
